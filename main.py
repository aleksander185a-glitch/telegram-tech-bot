...
async def handle_description(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await send_to_admin(context.bot, user_info, user_id)
        ...
        await update.message.reply_text("❌ Ошибка при отправке. Попробуйте еще раз: /start")
...
async def send_to_admin(bot, user_info, user_id):
...
async def cancel_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("❌ Заявка отменена. Напишите /start чтобы начать заново.")
    return ConversationHandler.END
